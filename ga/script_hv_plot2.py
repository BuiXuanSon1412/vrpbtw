...
IMAGE_DIR = "./img/hv_convergence_by_size"
...
def plot_size_convergence(num_nodes):
    """
    Plot average HV convergence for a problem size (across all distributions).
    ...
    try:
        # 1. Find global nadir for this size
        ...
        # 4. Formatting
        num_instances = len(DISTRIBUTIONS) * len(SEEDS)
        num_runs = len(RUNS)
        # plt.title(
        #    f"HV Convergence: {batch_name}\n"
        #    f"(avg over {num_instances} instances × {num_runs} runs = {num_instances * num_runs} data points)"
        # )
...
def plot_all_sizes_comparison():
    ...
    for idx, num_nodes in enumerate(NUM_NODES):
        ...
        try:
            # Find global nadir for this size
            global_nadir = find_global_nadir_for_size(base_path, num_nodes)

            # Collect HV data for all algorithms
            algo_hv_data = collect_hv_data_for_size(base_path, num_nodes, global_nadir)
            ...
            for algo in ALGORITHMS:
                ...
                # Plot mean line with shaded std area
                (line,) = ax.plot(aligned_gens, mean_hv, label=algo, linewidth=2)
                ax.fill_between(
                    aligned_gens,
                    mean_hv - std_hv,
                    mean_hv + std_hv,
            ...
            num_runs = len(RUNS)
            # ax.set_title(
            #    f"{batch_name} ({num_instances}×{num_runs} = {num_instances * num_runs} points)"
            # )
...
def main():
    """Plot HV convergence for all problem sizes."""
    print("=" * 80)
    print("Generating HV Convergence Plots by Problem Size")
    print("=" * 80)

    # Plot individual size plots
    # for num_nodes in NUM_NODES:
    #    plot_size_convergence(num_nodes)
    ...
    print("  - Individual plots: N100.png, N200.png, N400.png, N1000.png")
    print("  - Combined plot: ALL_SIZES_COMPARISON.png")
    print("=" * 80)
