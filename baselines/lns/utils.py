...
from ga.problem import Problem, Solution, Route
...
def _split_unverified_trips(seq):
    trips = []
    trip = []
    trip_sign = 0

    for idx, (_, mask) in enumerate(seq):
        if idx == 0 or not mask:
            if trip:
                trips.append(trip)
                trip = []
                trip_sign = 0
...
def _routing_verified(seq, problem: Problem):
    ...
    if trips:
        ...
        for launch in range(0, first):
            for land in range(last + 1, len(nodes)):
                ...
                d = cal_route_distance([nodes[idx][0] for idx in temp_trip], problem)
            ...
            if len(ll_trips) == len(trips):
                ...
                if temp_trips:
                    for trip in temp_trips:
                        ...
                        if land - launch > problem.drone_trip_duration:
                            drained = True
                            break
                ...
                if opt_cost > temp_cost and not drained:
...
def verify_drone_trips(seq, problem: Problem):
    if not seq:
        return []

    verified_seq = [(node, 0) for node, _ in seq]
    unverified_trips = _split_unverified_trips(seq)

    for trip in unverified_trips:
        sign = seq[trip[0]][1]
        accepted_seq = deepcopy(verified_seq)

        for prefix_len in range(1, len(trip) + 1):
            candidate_seq = deepcopy(accepted_seq)
            for seq_idx in trip[:prefix_len]:
                candidate_seq[seq_idx] = (candidate_seq[seq_idx][0], sign)

            route, _, _, _ = _routing_verified(candidate_seq, problem)
            if not route:
                break

            verified_seq = candidate_seq
            accepted_seq = candidate_seq

    return verified_seq


def routing(seq, problem: Problem, return_seq=False):
    verified_seq = verify_drone_trips(seq, problem)
    result = _routing_verified(verified_seq, problem)

    if return_seq:
        return (*result, verified_seq)

    return result


def _write_partitioned_masks(chro, seqs, problem: Problem):
    n_node = len(problem.nodes)
    seq_idx = 0
    gene_idx = 0
    ...
    for i in range(len(chro[0])):
        ...
        if seq_idx < len(seqs) and gene_idx < len(seqs[seq_idx]):
            chro[1][i] = seqs[seq_idx][gene_idx][1]
        gene_idx += 1
